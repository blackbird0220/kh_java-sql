...
update sales set money=(select bprice from book where bid=sales.bno)*amount; --MySQL과 다름
...
update book2 set bcount = 5 where bkind = 'HC';

--19) 도서2(book2) 테이블에서 도서 분류가 TC인 튜플을 제거하시오.
delete from book2 where bkind='TC';
...
updata book2 set ckdata='재입고요망' where bcount<=7;
...
create table bkind(kindcode varchar(6) primary key, kindname varchar(50));
...
update book2 set bcount=bcount+5 where pubdate between '2022-08-01' and '2022-08-31';

--25) 회원(member) 테이블에서 모든 회원의 모든 정보를 조회하시오.
select * from member;

--26) 판매(sales) 테이블에서 구매한 적이 있는 회원의 아이디를 중복을 제거하여 조회하시오. distinct 중복제거
select distinct id, pw, name from member where id in (select id from sales);
select distinct id, pw, name from member where id not in (select id from sales);
select * from member where id in select id from sales);
select * from member where id not in select id from sales);
...
select btitle, author, bcount from book where pubcom = '한빛미디어' or pubcom = '남가람북스';
...
select bkid, btitle, author from book where bkind not in('IT', 'NV', 'TC') order by pubdate desc;
...
select sno, bno, id  (from sales order by money desc) where  money <=3;
...
create view view_sales as select sno, bno, id, money from sales ;
...
create view book_view as select bkind, btitle bprice, pubcom from book2 where bkind in ('IT', 'TC', 'HC');
...
select btitle from book where bid not in (select bno from sales);
...
select distinct a.btitle, a.bprice, b.amount, b.money from book a inner join sales b on a bid=b.bno;
...
--select a.btitle, a.bprice, a.author from book a lefr outer join sales b on a.bookid=b.bno; ?????

select distinct a.btitle, a.bookprice, a.author from book a left outer join sales b on a.bid=b.bno
minus select distinct a.btitle, a.bprice, a.author from book a right out sales b on a.bid=b.bno;

select btitle, bprice, author from book where bid in (select bid from book minus select bno from sales);
...
minus select distinct a.id, a.name from member a right outer join sales b on a.id=b.id;
...
create view minus_book_view as (select bid, bkind, btitle, price 
from book MINUS select bid, bkind, btitle, bprice from book2);
